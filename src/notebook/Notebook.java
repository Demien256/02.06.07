...
public class Notebook {
    ...
    public List<Note> getNotesForDay(LocalDateTime dateTime) {
        return notes.stream()
                .filter(note ->
                        note.dateTime().toLocalDate().isEqual(dateTime.toLocalDate()))
                .sorted(Comparator.comparing(Note::dateTime))
    ...
    public List<Note> getNotesForWeek(LocalDateTime startOfWeek) {
        ...
        return notes.stream()
                .filter(note ->
                        !note.dateTime().isBefore(startOfWeek) &&
                                !note.dateTime().isAfter(endOfWeek))
                .sorted(Comparator.comparing(Note::dateTime))
                .collect(Collectors.toList());
    }
